...
import { useAdSetsPlatformsChangeStatusRunAndPauseByUserMutation } from '../../../../apis/ads-manager-api-slice';
...
const AdSetUnGroupList = () => {
    ...
    const {
        extra_amViewBy,
        extra_amAdSetUnGroupColumnNameSort,
        extra_amAdSetUnGroupColumnOrderSort,
    ...
    const dataResult = useMemo(
        () =>
            adsManagerData?.ad_sets_platforms.map((asp) => {
                const ad_set_current = adsManagerData.ad_sets.filter((as) => as.id === asp.ad_set);
                ...
                return {
                    ...
                    run_on: adsManagerData.ads_platforms.some(
                        (adp) => adp.ad_set_id === asp.ad_set && adp.run_on && adp.publisher_platform === asp.publisher_platform
                    ),
                    is_Campaign_Paused: !adsManagerData.campaigns_platforms
                        .filter((cp) => cp.campaign === ad_set_current[0].campaign && cp.publisher_platform === asp.publisher_platform)
                        .some((cp) => cp.status !== statusTypes.PAUSED),
                    is_Ads_Paused: !adsManagerData.ads_platforms
                        .filter((ap) => ap.ad_set_id === asp.ad_set && ap.publisher_platform === asp.publisher_platform)
        ...
        if (filteredDataResult?.length) {
            ...
            if (searchQuery.trim()) {
                filteredDataResult = filteredDataResult.filter((item) =>
                    item.ad_set_current[0].name.toLowerCase().includes(searchQuery.toLowerCase())
                );
            }

            if (isHideEmptyRows) {
                filteredDataResult = filteredDataResult.filter((item) => item.ad_sets_performance.length > 0);
            }

            if (extra_amSelectedCampaignPlatformList.length) {
                filteredDataResult = filteredDataResult.filter((item) =>
                    extra_amSelectedCampaignPlatformList.some((i) => i.campaign === item.ad_set_current[0].campaign)
                );
            }
        }

        dispatch_ExtraLC({
            type: actions_ExtraLAT.AT_SET_EXTRA_ADS_MANAGER_AD_SET_UN_GROUP_LIST,
            payload: filteredDataResult
        });
        setProducts(filteredDataResult);
    }, [dataResult, searchQuery, isHideEmptyRows, extra_amSelectedCampaignPlatformList, isAuthencatedGoogle, isAuthencatedFacebook, isAuthencatedInstagram, isAuthencatedLinkedin, isAuthencatedPinterest, isAuthencatedSnapchat, isAuthencatedTiktok, isAuthencatedMeta, isAuthencatedMessenger, isAuthencatedYoutube, dispatch_ExtraLC]);
    ...
    const handleSelectionChange = (value) => {
        dispatch_AdsManagerLC({
            type: actions_AdsManagerLAT.AT_SET_ADS_MANAGER_AD_SET_PLATFORM_SELECTED_LIST,
            payload: value
        });
        dispatch_ExtraLC({
            type: actions_ExtraLAT.AT_SET_EXTRA_ADS_MANAGER_AD_SET_SELECTED_PLATFORM_LIST,
            payload: value
        });
        if (value && value.length > 0) {
            ...
            if (ids.length > 0) {
                ...
                dispatch_ExtraLC({
                    type: actions_ExtraLAT.AT_SET_EXTRA_ADS_MANAGER_AD_SET_SELECTED_PLATFORM_LIST_STRING,
                    payload: ids
                });
            }
        } else {
    ...
    const Name_parentBodyTemplate = (rowData, props_column) => {
        ...
        return (
            <div className="Name_parentBodyTemplate">
                <div className="Name_field">{rowData.ad_set_current[0].name}</div>
                <Form.Group className="mb-1 Name_edit">
                    <div
                        className={`switch ${
                            rowData.status === statusTypes.ACTIVE && (rowData.is_Campaign_Paused || rowData.is_Ads_Paused)
                                ? 'switch-warning'
                                : 'switch-success'
                        } d-inline m-r-10`}
                    >
                        <Form.Control
                            type="checkbox"
                            style={{ width: 'auto' }}
                            id={'checkRunPauseParent_AdSet_' + platform_id}
                        ...
                        <Form.Label
                            ...
                            htmlFor={'checkRunPauseParent_AdSet_' + platform_id}
                            className="cr"
                        />
                    </div>
                    <div onClick={() => callAdSetEditModal(rowData.ad_set_current[0])} style={{ cursor: 'pointer' }}>
    ...
    const Clicks_parentBodyTemplate = (rowData) => {
        if (rowData.ad_sets_performance.length === 0) return 0;
        return rowData.ad_sets_performance[0].clicks;
    };

    const Impressions_parentBodyTemplate = (rowData) => {
        if (rowData.ad_sets_performance.length === 0) return 0;
        return rowData.ad_sets_performance[0].impressions;
    };

    const Spent_parentBodyTemplate = (rowData) => {
        if (rowData.ad_sets_performance.length === 0) return '$0';
        return formatCurrency(rowData.ad_sets_performance[0].spend);
    };
    ...
    const CPM_parentBodyTemplate = (rowData) => {
        if (rowData.ad_sets_performance.length === 0) return '$0';
        let spend = parseFloat(rowData.ad_sets_performance[0].spend);
        let impre = parseInt(rowData.ad_sets_performance[0].impressions);
    ...
    const CTR_parentBodyTemplate = (rowData) => {
        if (rowData.ad_sets_performance.length === 0) return '0%';
        let clicks = parseFloat(rowData.ad_sets_performance[0].clicks);
        let impre = parseInt(rowData.ad_sets_performance[0].impressions);
    ...
    const CPA_parentBodyTemplate = (rowData) => {
        if (rowData.ad_sets_performance.length === 0) return '$0';
        let actions = parseFloat(rowData.ad_sets_performance[0].actions);
        let spend = parseFloat(rowData.ad_sets_performance[0].spend);
    ...
    const ROAS_parentBodyTemplate = (rowData) => {
        if (rowData.ad_sets_performance.length === 0) return '$0';
        let earned = parseFloat(rowData.ad_sets_performance[0].earned);
        let spend = parseFloat(rowData.ad_sets_performance[0].spend);
        let tempResult = spend === 0 ? 0 : earned / spend;
        return formatCurrency(tempResult);
    };

    const toggleHandlePlatform = async (event, ad_set_platform_id) => {
        const isChecked = event.target.checked;

        const data = {
            status: !isChecked ? statusTypes.PAUSED : statusTypes.ACTIVE,
            ad_set_platform_ids: [ad_set_platform_id],
            disabled: !isChecked
        };
    ...
    const Name_sort = (data) => {
        return data.ad_set_current[0].name;
    };
        ...
        if (products && products.length > 0) {
            ...
            for (let rowData of filteredArray) {
                ...
                if (field === 'roas') {
                    ...
                    result += parseFloat(rowData.ad_sets_performance.length > 0 ? rowData.ad_sets_performance[0].spend : 0);
                    fieldName = 'SPENT';
                } else if (field === 'impre') {
                    result += rowData.ad_sets_performance.length > 0 ? rowData.ad_sets_performance[0].impressions : 0;
                    fieldName = 'IMPR';
                } else if (field === 'clicks') {
                    result += rowData.ad_sets_performance.length > 0 ? rowData.ad_sets_performance[0].clicks : 0;
                    fieldName = 'CLICKS';
                }
                total += result;
            }
    ...
    const calculateTotalOthers = (field) => {
        ...
        if (products && products.length > 0) {
            ...
            for (let rowData of filteredArray) {
                total_Clicks += rowData.ad_sets_performance.length > 0 ? rowData.ad_sets_performance[0].clicks : 0;
                total_Impre += rowData.ad_sets_performance.length > 0 ? rowData.ad_sets_performance[0].impressions : 0;
                total_Spent += parseFloat(rowData.ad_sets_performance.length > 0 ? rowData.ad_sets_performance[0].spend : 0);
            }
    ...
    const footer = () => {
        ...
        return (
            <div style={{ display: 'flex', justifyContent: 'flex-start', alignItems: 'center' }}>
                <div style={{ width: '720px', fontWeight: 'bolder' }}>{`Totals for ${products && products.length} Ad Sets`}</div>
    ...
    const handleSortChange = (event) => {
        ...
        dispatch_ExtraLC({
            type: actions_ExtraLAT.AT_SET_EXTRA_ADS_MANAGER_AD_SET_UN_GROUP_COLUMN_NAME_SORT,
            payload: event.sortField
        });
        dispatch_ExtraLC({
            type: actions_ExtraLAT.AT_SET_EXTRA_ADS_MANAGER_AD_SET_UN_GROUP_COLUMN_ORDER_SORT,
    ...
    return (
        <div>
            <DataTable
                ...
                selection={extra_amSelectedAdSetPlatformList}
                ...
                scrollHeight="800px"
                sortField={extra_amAdSetUnGroupColumnNameSort}
                sortOrder={extra_amAdSetUnGroupColumnOrderSort}
...
export default AdSetUnGroupList;
